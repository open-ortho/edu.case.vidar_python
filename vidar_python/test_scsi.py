...
class TestScsi(unittest.TestCase):
    ...
    def test_inquiry_result(self):
        ...
        parsed_data = scsi.parse_scsi_inquiry_response(response_bytes)
        print(json.dumps(parsed_data, indent=4))
        self.assertEqual(parsed_data.get("Vendor ID"),"VIDAR")
        self.assertEqual(parsed_data.get("Product ID"),"VXR-12")

        # result_hex_string = "55 53 42 53 01 00 00 00 00 00 00 00 80 00 06 12 00 00 00 34 00 00 00 00 00 00 00 00 00 00 00 20 34 39 2e 37 70 00 00 00 10 00 00 00 00 00 00 00 00 00 00 00"
        # response_bytes = bytes.fromhex(result_hex_string.replace(" ", ""))
        # parsed_data = scsi.parse_scsi_inquiry_response(response_bytes)
        # print(json.dumps(parsed_data, indent=4))