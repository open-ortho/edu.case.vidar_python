    ...
    public class Scan
    {
        public unsafe static int scan(_DIGITIZERINFO digitizerInfo, ScannerData scanner_data, ScanConfig config)
        {
            ...
            try
            {
                ...
                short dpiX = scan_parameters.getShort(OFFSET_DPI_X);
                short dpiY = scan_parameters.getShort(OFFSET_DPI_Y);
                prefix = prefix.Replace("${DPI}", dpiX.ToString());
                ...
                // Save as PNG
                writeImageToFile(imageBuffer, height, width, filePath, bitDepth, dpiX, dpiY);
